...
void printPoolContent () {
    ...
    for (i = 0; i < buf_capacity; i++) {
        printf("{ table %d:: index %d[%lu %s %d] }", q[i].tid, i, q[i].pagenum, (q[i].isDirty == true ? "O" : "X"),
               q[i].pinCount);
        printf("\n");
    }
}
...
//        i = head;
        ...
        while (true) {
            ...
            // if the frame's pinCount=0 and not a headerPage, select it for victim
            if(q[current].pinCount == 0 && q[current].pagenum != 0){
